...
function AcademicInfo() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const studentResponse = await apiClient.get(`/students/byId/${student_id}`);
        console.log("Student data loaded:", studentResponse.data);
        setStudentData(studentResponse.data);

        // Fetch academic info to check if student has previous records
        const academicResponse = await apiClient.get(`/academicInfo/byStudent/${student_id}`);
  ...
  const handleSubmit = (values, { resetForm }) => {
    ...
    apiClient.post("/academicInfo", newInfo)
      .then((postResponse) => {
        ...
        return apiClient.post("/academicPerformance", academicPerformanceData);
      })
      .then(() => {
