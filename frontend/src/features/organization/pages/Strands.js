...
import AddStrandModal from "../components/AddStrandModal";
...
function StrandSections() {
  ...
  const fetchStrands = useCallback(() => {
    apiClient
      .get(`/strands/byDepartment/${department_id}`)
      .then((res) => {
        setStrands(res.data);
  ...
  const fetchSections = useCallback((strandId = null) => {
    const url = strandId
      ? `/sections/byStrand/${strandId}`
      : `/sections/byDepartment/${department_id}`;
  ...
  const strandSchema = Yup.object().shape({
    strand_name: Yup.string().trim().max(20, "Use 20 characters or fewer").required("Strand name is required"),
    strand_description: Yup.string().trim().max(100, "Use 100 characters or fewer").required("Strand description is required"),
  });
  ...
  const handleCreateStrand = async (values) => {
    await apiClient.post("/strands", {
      ...values,
      department_id,
    });
    fetchStrands();
  ...
  const confirmDelete = async () => {
    try {
      await apiClient.delete(`/sections/${sectionToDelete.section_id}`);
  ...
  const confirmStrandDelete = async () => {
    try {
      ...
      const sectionsResponse = await apiClient.get(`/sections/byStrand/${strandToDelete.strand_id}`);
      const sections = sectionsResponse.data;

      // Delete all sections associated with this strand
      for (const section of sections) {
        await apiClient.delete(`/sections/${section.section_id}`);
      }

      // After all sections are deleted, delete the strand
      await apiClient.delete(`/strands/${strandToDelete.strand_id}`);
  ...
  return (
    <div className="container workspace-page strand-sections-page">
      ...
      <section className="strand-sections-panel">
          ...
          <div className="modal-overlay entity-edit-overlay" role="dialog" aria-modal="true" aria-labelledby="edit-strand-title">
            <div className="modal-content entity-edit-modal">
              ...
              <Formik
                ...
                onSubmit={(values, { resetForm }) => {
                  apiClient
                    .put(`/strands/${strandToEdit.strand_id}`, {
          ...
          <div className="modal-overlay entity-edit-overlay" role="dialog" aria-modal="true" aria-labelledby="edit-section-title">
            <div className="modal-content entity-edit-modal">
              ...
              <Formik
                ...
                onSubmit={(values, { resetForm }) => {
                  apiClient
                    .put(`/sections/${sectionToEdit.section_id}`, {
                      ...values,
        ...
        <AddStrandModal
          isOpen={showAddStrandModal}
          departmentName={departmentName}
          onClose={() => setShowAddStrandModal(false)}
          onSubmit={handleCreateStrand}
        />
    </div>
  );
}
