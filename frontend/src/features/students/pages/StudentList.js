...
function StudentList() {
    ...
    const fetchStudents = async () => {
      try {
        ...
        if (privileges?.sectionId) {
          response = await apiClient.get(`/students/section/${privileges.sectionId}`);
        } 
        // If user is a department user, fetch all students but filter by department
        else if (privileges?.departmentId) {
          response = await apiClient.get(`/students/department/${privileges.departmentId}`);
        }
  ...
  const handleEdit = async (student) => {
    try {
      ...
      const response = await apiClient.get(`/students/byId/${student.student_id}`);
  ...
  const handlePrint = async (student) => {
    try {
      ...
      const studentResponse = await apiClient.get(`/students/byId/${student.student_id}`);
      const studentData = studentResponse.data;

      // Fetch academic info
      const academicResponse = await apiClient.get(`/academicInfo/byStudent/${student.student_id}`);
