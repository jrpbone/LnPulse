...
function SectionStudents() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [studentsResponse, sectionResponse] = await Promise.all([
        apiClient.get(`/students/section/${targetSectionId}`),
        apiClient.get(`/sections/${targetSectionId}`),
      ]);
      ...
      const studentsWithLatestInfo = await Promise.all(
        studentsResponse.data.map(async (student) => {
          try {
            const academicInfoResponse = await apiClient.get(`/academicInfo/byStudent/${student.student_id}`);
  ...
  useEffect(() => {
    const fetchAcademicSettings = async () => {
      try {
        const settingsResponse = await apiClient.get("/academicSettings/current");
  ...
  const handleAcadModalSubmit = async (values) => {
    ...
    try {
      ...
      const promises = selectedStudents.map(async (studentId) => {
        ...
        const academicResponse = await apiClient.post("/academicInfo", academicInfoData);
        ...
        await apiClient.post("/academicPerformance", academicPerformanceData);
      });
